...
pt_model_path = "model_500.pt"  # Use the TorchScript file!
print(f" Loading TorchScript model from {pt_model_path}")
ts_model = torch.jit.load(pt_model_path)
ts_model.eval()
ts_model.to('cpu')  # <-- Ensure model is on CPU
...
dummy_observations = torch.ones(1, obs_dim, device='cpu')  # <-- Ensure input is on CPU
dummy_observation_history = torch.ones(1, obs_dim * obs_his_num, device='cpu')  # <-- Ensure input is on CPU

# === Step 3: Export to ONNX ===
onnx_model_path = "model_528.onnx"
torch.onnx.export(
    ts_model,
    (dummy_observations, dummy_observation_history),  # Pass as tuple!
    onnx_model_path,
...
with torch.no_grad():
    ts_output = ts_model(dummy_observations, dummy_observation_history).numpy()

# ONNXRuntime inference
ort_sess = ort.InferenceSession(onnx_model_path)
ort_inputs = {
    "observations": dummy_observations.numpy(),
    "observation_history": dummy_observation_history.numpy()
}
