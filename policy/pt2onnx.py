...
def load_state_dict_any(ckpt_path: str, device="cpu"):
    ...
    # Nested containers commonly produced by trainers
    ...
    # TorchScript archives (RecursiveScriptModule / ScriptModule, etc.)
    get_state = getattr(ckpt, "state_dict", None)
    if callable(get_state):
        state = get_state()
        if isinstance(state, dict):
            return state

    return ckpt  # fall back; caller will surface a useful error
